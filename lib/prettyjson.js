...
var renderToArray = function(data, options, indentation, elementLength) {
    ...
    data.forEach(function(element) {
      ...
      // If the element is an array or object, render it in next line
        ...
        outputArray.push.apply(
          outputArray,
          renderToArray(
            element, options, indentation + options.defaultIndentation, --elementLength)
        );
      }
    });
  ...
  Object.getOwnPropertyNames(data).forEach(function(i) {
    ...
    if (isSerializable(data[i], false, options)) {
      ...
      key += renderToArray(data[i], options, nextIndentation, --elementLength)[0];
      output.push(key);
      ...
      if (Array.isArray(data[i])) {
        preToken = '[ ';
        postToken = ']' + elementLength;
      } else { // is object
        preToken = ' {';
        postToken = '}' + elementLength;
      }

      if (elementLength === undefined || Number.isInteger(elementLength) && elementLength > 1) {
      ...
      output.push.apply(
        ...
        renderToArray(
          data[i],
          options,
          indentation + options.defaultIndentation, data[i].length, --elementLength
        )
      );
      if (options.copyable) {
...
exports.render = function render(data, options, indentation) {
  ...
  var result = renderToArray(data, options, indentation, Object.keys(data).length);
